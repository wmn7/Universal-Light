'''
@Author: WANG Maonan
@Date: 2023-03-01 18:16:41
@Description: 感应控制
@LastEditTime: 2023-03-01 18:45:55
'''
import gym
import logging
import numpy as np
...
class env_wrapper(gym.Wrapper):
    ...
    def reset(self, ):
        ...
        if (self.mode == 'train') or (self.reset_num == 0):
            ...
            _observations = observations[self.tls_id] # 当前路口的 obs
            ...
            observations = self._process_obs(_observations)
        else:
            observations = 1
    ...
    def step(self, action):
        """是否切换, keep->True, bool(1), change->False, bool(0)
        action 只有两个值可以选择:
        -> 0 表示切换到下一个信号灯
        -> 1 表示继续保持
        这里判断的逻辑就是，当前 phase 对应的 movement 最大的占有率 20% 就 keep，否则就 change

        Args:
            action (int): 设置是否保持或是改变当前的相位
        """
        action =  {self.tls_id: action}

        observations, rewards, dones, info = self.env.step(action)
        process_observation = self._process_obs(observations[self.tls_id])
        return process_observation, rewards, dones['__all__'], info
    ...
    def _process_obs(self, observation):
        ...
        for _movement_id, _movement in enumerate(self.net_movements[self.env._net]): # 按照 movment_id 提取
            ...
            if (_max_occ < max_occupancy) and is_now_phase:
                _max_occ = max_occupancy
