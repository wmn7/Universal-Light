...
@Description: 对 obs 进行堆叠, 同时模拟 obs delayed 的情况
- 例如现在要对叠 K 个 obs，delay 是 D 个（也就是 D×5 s）
- 于是队列长度是 K+D，每次从中取 K 个
参考链接为，https://github.com/openai/gym/blob/master/gym/wrappers/frame_stack.py
...
class frame_stack_delayed_wrapper(gym.ObservationWrapper):
    ...
    def __init__(
        ...
        Args:
            ...
            delayed_num (int): The number of delayed frame
            lz4_compress (bool): Use lz4 to compress the frames internally
        """
        super().__init__(env)
        self.num_stack = num_stack
        self.num_delayed = num_delayed
        self.lz4_compress = lz4_compress

        self.frames = deque(maxlen=(num_stack+num_delayed)) # 队列
    ...
    def observation(self, observation):
        ...
        """
        assert len(self.frames) == (self.num_stack+self.num_delayed), (len(self.frames), self.num_stack+self.num_delayed)
        return LazyFrames(list(self.frames)[:self.num_stack], self.lz4_compress)
    ...
    def reset(self, **kwargs):
        ...
        # reset 的时候将初始 obs 重复 num_stack 次, 来保证队列填充完毕
        [self.frames.append(obs) for _ in range(self.num_stack+self.num_delayed)]
